...
APP_DIR="${APP_DIR:-/var/www/aishwani-tayal}"
SERVICE_NAME="${SERVICE_NAME:-aishwani-tayal}"
PORT="${PORT:-8010}"
...
ARCHIVE_NAME="aishwani-tayal-deploy.tar.gz"
REMOTE_ARCHIVE="/tmp/${ARCHIVE_NAME}"
LOCAL_ARCHIVE="$(mktemp -t aishwani-tayal-deploy.XXXXXX.tar.gz)"
...
$SUDO "$APP_DIR/.venv/bin/python" manage.py collectstatic --noinput

echo "Writing systemd service"
$SUDO tee "/etc/systemd/system/${SERVICE_NAME}.service" >/dev/null <<EOF
[Unit]
Description=Aishwani Tayal Django app
...
if curl -fsSI --max-time 15 "http://${SERVER_NAME}:${PORT}" >/dev/null; then
